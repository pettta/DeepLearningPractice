...
print("X_TRAIN", X_train) # TODO REMOVE
print("X_TEST", X_test) # TODO REMOVE
print("Y_TRAIN", Y_train) # TODO REMOVE
print("Y_TEST", Y_test) # TODO REMOVE
...
L2 = FullyConnectedLayer(X_train.shape[1], 10) # 0-9
xavier_param = np.sqrt(6) / np.sqrt(X_train.shape[1] + 10)
L2.setWeights(np.random.uniform(-xavier_param, xavier_param, (X_train.shape[1], 10)))
L2.setBiases(np.random.uniform(0, 0, (1, 10)))
L3 = SoftmaxLayer()
L4 = CrossEntropy()
layers=[L1, L2, L3, L4]
...
for i in range(10000):
    ...
    print("CROSS ENTROPY TRAIN LOSS", current_centropy_train_loss) # TODO REMOVE
    print("CROSS ENTROPY TEST LOSS", current_centropy_test_loss) # TODO REMOVE
    ...
    if centropy_train_loss_diff <= 1e-5 and centropy_test_loss_diff > 0:
        print("CONVERGED")
        break 
    ...
    for layer in reversed(layers[:-1]):
        ...
        if(isinstance(layer, FullyConnectedLayer)):
            layer.updateWeights(grad, 1e-3, True, rho1, rho2, i) # NOTE: I should move this elsewhere / think up better design 
        grad = newGrad
...
plt.show()
plt.pause(2) 
plt.savefig(f"hw4_part6_cross_entropy_plot.png")
