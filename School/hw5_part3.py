...
L3 = ReLuLayer()
L4 = FullyConnectedLayer(100, 1000) # Hidden layer 2
L5 = ReLuLayer()
L6 = FullyConnectedLayer(1000, 10)
L7 = SoftmaxLayer()
L8 = CrossEntropy()
...
for i in range(100000):
    ...
    if (abs(centropy_train_loss_diff) <= 1e-9 and abs(centropy_train_loss_diff)!=0) and centropy_test_loss_diff > 0 : # changed stopper for overfitting due to this being about causing overfitting 
    ...
    for layer in reversed(layers[:-1]):
        ...
        if(isinstance(layer, FullyConnectedLayer)):
            layer.updateWeights(grad, learningRate=lr, useAdam=True, rho1=rho1, rho2=rho2, t=i)
        grad = newGrad
...
plt.show()
plt.pause(2) 
plt.savefig(f"hw5_part3_cross_entropy_plot.png")
