...
L2 = FullyConnectedLayer(X_train.shape[1], 100) # Hidden layer 1
L3 = DropoutLayer(blend) 
L4 = ReLuLayer() 
L5 = FullyConnectedLayer(100, 1000) # Hidden layer 2
#L6 = DropoutLayer(blend) worse with this in 
L7 = ReLuLayer()
L8 = FullyConnectedLayer(1000, 10)
L9 = SoftmaxLayer()
L10 = CrossEntropy()
layers=[L1, L2, L3, L4, L5, L7, L8, L9, L10]
...
for i in range(100000):
    ...
    if i % 100 == 0: # print only every 100 iterations to keep console clutter low
        print("CROSS ENTROPY TESTING LOSS DIFFERENCE", centropy_test_loss_diff)
        print("CROSS ENTROPY TRAINING LOSS DIFFERENCE", centropy_train_loss_diff)
        
    if (abs(centropy_train_loss_diff) <= 1e-8 and abs(centropy_train_loss_diff)!=0) and centropy_test_loss_diff > 0 :
        print("BROKE ON TRAINING LOSS DIFFERENCE", centropy_train_loss_diff)
        print("CONVERGED")
        break 
    ...
    for layer in reversed(layers[:-1]):
        ...
        if(isinstance(layer, FullyConnectedLayer)):
            layer.updateWeights(grad, learningRate=lr, useAdam=True, rho1=rho1, rho2=rho2, t=i, useL2=True, l2alpha=1e-2)
        grad = newGrad
...
plt.show()
plt.pause(2) 
plt.savefig(f"hw5_part4_cross_entropy_plot.png")
